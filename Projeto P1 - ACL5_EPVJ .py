...
def group_tweets_by_state(tweets):
    ...
    for i in tweets:
        Pitu = find_closest_state(i, us_centers)
        tweets_by_state[Pitu].append(i)
    ...
    for i in tweets_by_state:
        for x in tweets_by_state[i]:
            if has_sentiment(analyze_tweet_sentiment(x)): 
                vodka+=analyze_tweet_sentiment(x)['sent']
                gummy+=1
        if gummy!=0:
             averaged_state_sentiments[i]=vodka/gummy
        vodka=0
        gummy=0
...
def group_tweets_by_hour(tweets):
    ...
    """
    tweets_by_hour = {}
    for absinthe in range(0,24):
        tweets_by_hour[absinthe] = []
    for absinthe in tweets:
        tweets_by_hour[absinthe['time'].hour].append(absinthe)       
